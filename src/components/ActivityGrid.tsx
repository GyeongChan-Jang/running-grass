...
function CellContent({ distance, isFirstDayOfMonth, date, showDistance }: CellContentProps) {
  return (
...
export default function ActivityGrid({ totalDistance }: ActivityGridProps) {
  ...
  return (
    <div className="relative pb-16">
      <div ref={gridRef} className="space-y-4 bg-white dark:bg-gray-900 p-4 rounded-lg">
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-semibold flex items-center gap-2">
            <Image src="/images/grass.png" alt="러닝 잔디" width={24} height={24} />
        ...
        <div className="grid grid-cols-7 gap-1">
          {['일', '월', '화', '수', '목', '금', '토'].map((day) => (
            <div key={day} className="text-center text-md md:text-lg text-gray-500 dark:text-gray-400">
              {day}
            </div>
          ))}
        </div>
                  ...
                  return (
                    <div key={dateStr} className="relative group p-0.5">
                      <div
                        ...
                        title={`${format(date, 'M월 d일')}: ${distance.toFixed(1)}km`}
                      >
                        <CellContent
                          distance={distance}
        ...
        <div className="flex items-center justify-end gap-2">
          <span className="text-xs md:text-sm text-gray-600 dark:text-gray-400">짧게</span>
          {[0, 3, 7, 12, 20].map((value) => (
            <div
              key={value}
              className={`w-3 h-3 md:w-4 md:h-4 rounded-sm ${getIntensityColor(value)}`}
              title={`${value}km`}
            />
          ))}
          <span className="text-xs md:text-sm text-gray-600 dark:text-gray-400">길게</span>
        </div>
      </div>
      ...
      <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-10">
        ...
        >
          ...
          <span className="text-lg md:text-xl font-bold mt-1">나의 잔디밭 공유하기</span>
        </Button>
      </div>
    </div>
  )
